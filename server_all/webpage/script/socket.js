var socket = io.connect('62.249.189.153:2520', {secure: false}); //This line declares a socket.io object to var "socket" and connects to the server (change the IP-address and port to your own)
...
function toggleSlider(this_id) {
    var checkState;
    document.getElementById(this_id.id.replace('chk'));
    if(this_id.checked) {
        checkState = 1;
    }else{
        checkState = 0;
...
function changeLEDState(el_id){
    var myvar = toggleSlider(el_id);
    socket.emit('changeLEDState', myvar); //Here the actual socket-object function is called. If we want a response we will have to set up a function (.on) like earlier.
    console.log("changeLEDState called with value: "+myvar);
}
...
function requestDataFromBoard(el_id,interval) {
    var myvar = toggleSlider(el_id);
    if (myvar >= 1){
