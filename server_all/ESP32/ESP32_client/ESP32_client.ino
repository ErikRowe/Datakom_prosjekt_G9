...
void changeLEDState(const char * LEDStateData, size_t length) { //What happens when the ESP32 receives a instruction from the server (with variable) to change the LED
  ...
  int loc_interval=500; //endra nettop fra unsigned long
  Serial.println(prev_LEDState);
  while(LEDState != prev_LEDState)
    {
      if (LEDState == 1){
        while(millis() < prev_millis + loc_interval){
          digitalWrite(33,HIGH);
          }
        digitalWrite(18, HIGH);//We now use the varible to change the light (1 is on, 0 is off)
        digitalWrite(19,LOW);
        digitalWrite(33,LOW);    
      }else{
        while(millis() < prev_millis + loc_interval){
        ...
        digitalWrite(19, HIGH);//We now use the varible to change the light (1 is on, 0 is off)
        digitalWrite(18,LOW);
        digitalWrite(32,LOW);
        }
...
void dataRequest(const char * DataRequestData, size_t length) {//This is the function that is called everytime the server asks for data from the ESP32
  ...
  if(RequestState == 0) { //If the datarequest gives the variable 0, do this (default)
    ...
    itoa(analogRead(35), str, 10); //Use a special formatting function to get the char array as we want to, here we put the analogRead value from port 35 into the str variable
...
void setup() {
    ...
    //webSocket.on("DoorControl",controlDoor);
